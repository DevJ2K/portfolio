<template>
	<NavigationBar/>
	<main class="dark relative z-10 flex size-full min-h-screen w-screen flex-col items-center overflow-y-hidden bg-computorv1-app-bg font-Roboto">
		<div class="h-24"></div>
		<HomeView v-if="$route.name === 'computorv1'"/>
		<LearnMathView v-if="$route.name === 'computorv1/learn-math'"/>
		<TestView v-if="$route.name === 'computorv1/playground'"/>
	</main>
</template>
...
import LearnMathView from './views/LearnMathView.vue';
