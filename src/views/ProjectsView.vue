<template>
	<main class="relative flex size-full min-h-screen flex-col items-center">
		...
		<main class=" container z-10 flex min-h-screen flex-col items-center gap-9 text-low-contrast-text dark:text-d-low-contrast-text ">
			...
			<div v-else-if="projectList.filter((project) => containsSearch(project)).length > 0" v-for="project in projectList.filter((project) => containsSearch(project))" :key="project" class="flex w-full flex-col items-center gap-8">
				<ProjectCard :title="project.name" :description="project.description" :tags="project.tags" :github-link="project.github" :project-link="project.link"/>

			</div>
			<div v-else class="flex size-full flex-col items-center justify-center gap-2 text-high-contrast-text dark:text-d-high-contrast-text">
				<svg class="size-14 md:size-20" width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10 6.5C10 8.433 8.433 10 6.5 10C4.567 10 3 8.433 3 6.5C3 4.567 4.567 3 6.5 3C8.433 3 10 4.567 10 6.5ZM9.30884 10.0159C8.53901 10.6318 7.56251 11 6.5 11C4.01472 11 2 8.98528 2 6.5C2 4.01472 4.01472 2 6.5 2C8.98528 2 11 4.01472 11 6.5C11 7.56251 10.6318 8.53901 10.0159 9.30884L12.8536 12.1464C13.0488 12.3417 13.0488 12.6583 12.8536 12.8536C12.6583 13.0488 12.3417 13.0488 12.1464 12.8536L9.30884 10.0159Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
				<h1 class=" text-center text-xl md:text-2xl font-medium">No results for {{ searchText }}.</h1>
			</div>
		</main>
